...
*
* \file            Lesson23.c
* \author          Jeff Molofee (NeHe) & GB Schmick 2000 & TipTup originely, Modified for LCCWin32 compiler by Robert Wishlaw 2002/11/24
* \author          After adapted by Thierry Dechaize to verify paradigm : one source for multiple C Compilers
* \version         2.0.1.0
* \date            15 février 2022
* \brief           Ouvre une fenêtre Windows et dessine un environnement (à partir d'une image) qui se reflète sur un cylindre.
...
#if defined __LCC__ || defined __CYGWIN__ || defined __POCC__ || defined __WATCOMC__      // LCC and CYGWIN needs this header File For timeGetTime
#include <mmsystem.h>
#endif
...
GLuint	texture[6],texid[6];// Storage For 6 Textures (MODIFIED)
...
* \details    En entrée le nom du fichier Bitmap et en sortie le tableau des six textures utilisables avec OpenGL
* \param	  szFileName1			nom du premier fichier BMP				                            *
* \param	  szFileName2			nom du deuxième fichier BMP				                            *
* \param	  *texid			    un pointeur sur le tableau des six textures générées			    *
* \return     bool                  un booleen, le tableau des six textures créé ou non.                *
*
*/
...
bool NeHeLoadBitmap2(LPTSTR szFileName1, LPTSTR szFileName2, GLuint *texid)					// Creates Texture From A Bitmap File
{
    ...
    glGenTextures(6, &texid[0]);					                    // Create Three Textures
    ...
    glPixelStorei(GL_UNPACK_ALIGNMENT, 4);								// Pixel Storage Mode (Word Alignment / 4 Bytes)
        ...
        gluBuild2DMipmaps(GL_TEXTURE_2D, 3, BMP1.bmWidth, BMP1.bmHeight, GL_BGR_EXT, GL_UNSIGNED_BYTE, BMP1.bmBits);
        ...
        glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER,GL_LINEAR);
        glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER,GL_LINEAR_MIPMAP_NEAREST);
        gluBuild2DMipmaps(GL_TEXTURE_2D, 3, BMP2.bmWidth, BMP2.bmHeight, GL_BGR_EXT, GL_UNSIGNED_BYTE, BMP2.bmBits);
...
int LoadGLTextures()										// Load Bitmaps And Convert To Textures
{
	...
	if (!NeHeLoadBitmap2("../../Data/BG1.bmp","../../Data/Reflect.bmp", texture))		// Load The Bitmap
		return FALSE;
...
/**	            This Code draw cube
* \brief      Fonction glDrawCube : Affichage d'un cube
* \details    Rien
* \return     GLvoid        un void OpenGL.
*
*/
